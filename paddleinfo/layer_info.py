...
try:
    ...
    def is_lazy(param: paddle.nn.parameter.Parameter) -> bool:
        del param
        return False


DETECTED_INPUT_OUTPUT_TYPES = Union[
    Sequence[Any], Dict[Any, paddle.Tensor], paddle.Tensor
]
...
class LayerInfo:
    ...
    def __init__(
        ...
        self.depth = depth
        self.depth_index: int | None = None
        self.children: list[LayerInfo] = []
        self.executed = False
    ...
    def calculate_size(
        ...
        if inputs is None:
            ...
            inputs_ = paddle.from_numpy(inputs)
            ...
            raise TypeError(
                f"Model contains a layer with an unsupported input or output type: {inputs}, type: {type(inputs)}"
            )
        return size, elem_bytes

    @staticmethod
    def get_param_count(
        module: paddle.nn.Layer, name: str, param: paddle.Tensor
    ) -> tuple[int, str]:
        """
        ...
        if name.endswith("_orig"):
            ...
            if pruned_weights is not None:
                parameter_count = int(paddle.sum(pruned_weights))
                return parameter_count, without_suffix
        return param.size, name

    @staticmethod
    def get_kernel_size(module: paddle.nn.Layer) -> (int | list[int] | None):
    ...
    @staticmethod
    def get_groups(module: paddle.nn.Layer) -> (int | None):
...
def nested_list_size(inputs: (Sequence[Any] | paddle.Tensor)) -> tuple[list[int], int]:
    ...
    if hasattr(inputs, "tensors"):
        ...
        inputs_paddle = paddle.from_numpy(inputs)
        size, elem_bytes = list(inputs_paddle.size()), inputs_paddle.element_size()
...
def prod(num_list: (Iterable[int] | paddle.Size)) -> int:
    result = 1
...
def rgetattr(module: paddle.nn.Layer, attr: str) -> (paddle.Tensor | None):
    ...
    assert isinstance(module, paddle.Tensor)
    return module
