...
LAYER_MODULES = (paddle.nn.MultiHeadAttention,)
WRAPPER_MODULES = ()
INPUT_DATA_TYPE = Union[paddle.Tensor, np.ndarray, Sequence[Any], Mapping[str, Any]]
CORRECTED_INPUT_DATA_TYPE = Optional[Union[Iterable[Any], Mapping[Any, Any]]]
INPUT_SIZE_TYPE = Sequence[Union[int, Sequence[Any], paddle.Size]]
CORRECTED_INPUT_SIZE_TYPE = List[Union[Sequence[Any], paddle.Size]]
...
def summary(
    ...
    dtypes: (list[paddle.dtype] | None) = None,
    mode: str = "same",
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        input_size (Sequence of Sizes):
                Shape of input data as a List/Tuple
        ...
        batch_dim (int):
                ...
                is specified, paddleinfo uses a batch size of 1 for the forward pass.
                Default: None
        ...
        device (paddle.core.Place):
                Uses this paddle device for model and input_data.
        ...
        dtypes (List[paddle.dtype]):
                If you use input_size, paddleinfo assumes your input uses FloatTensors.
    ...
    Return:
        ModelStatistics object
                See paddleinfo/model_statistics.py for more information.
    """
...
def process_input(
    ...
    if input_data is not None:
        ...
        if isinstance(x, (paddle.Tensor, np.ndarray)):
            x = [x]
    if input_size is not None:
        assert device is not None
        if dtypes is None:
            dtypes = [paddle.float32] * len(input_size)
...
def forward_pass(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to run paddleinfo. See above stack traces for more details. Executed layers up to: {executed_layers}"
        ) from e
    finally:
        if hooks:
...
def validate_user_params(
    ...
    if neither_input_specified and not_allowed:
        raise ValueError(
            f"You must pass input_data or input_size in order to use columns: {not_allowed}"
        )
    if dtypes is not None and any(
        dtype in (paddle.float16, paddle.bfloat16) for dtype in dtypes
    ):
        if input_size is not None:
            warnings.warn(
                "Half precision is not supported with input_size parameter, and may output incorrect results. Try passing input_data directly.",
                stacklevel=2,
            )
        if device is not None and device.type == "cpu":
            warnings.warn(
                "Half precision is not supported on cpu. Set the `device` field or pass `input_data` using the correct device.",
                stacklevel=2,
            )
...
def traverse_input_data(
    ...
    if isinstance(data, paddle.Tensor):
        ...
        result = action_fn(paddle.from_numpy(data))
...
def set_device(data: Any, device: (paddle.device | None)) -> Any:
...
def get_device(
    model: paddle.nn.Layer, input_data: (INPUT_DATA_TYPE | None)
) -> (paddle.device | None):
    """
...
def get_input_data_sizes(data: Any) -> Any:
    """
    Converts input data to an equivalent data structure of list
    instead of tensors.
    """
...
def get_total_memory_used(data: CORRECTED_INPUT_DATA_TYPE) -> int:
    ...
    """Not Support auto convert *.storage, please judge whether it is Pytorch API and convert by yourself"""
    result = traverse_input_data(
        data,
        action_fn=lambda data: (
            np.prod(data.shape) * data.element_size()
        ),
        aggregate_fn=lambda data: (lambda d: sum(d.values()))
...
def get_input_tensor(
    ...
    for size, dtype in zip(input_size, dtypes):
        input_tensor = paddle.rand(shape=size)
        if batch_dim is not None:
            input_tensor = input_tensor.unsqueeze(dim=batch_dim)
        x.append(input_tensor.to(device).astype(dtype))
    return x
...
) -> Callable[[paddle.nn.Layer, Any], None]:
...
) -> Callable[[paddle.nn.Layer, Any, Any], None]:
...
def apply_hooks(
    ...
    dict[int, LayerInfo],
    dict[
        int, tuple[paddle.nn.layer.layers.HookRemoveHelper, paddle.nn.layer.layers.HookRemoveHelper]
    ],
]:
    """
    ...
    stack: list[tuple[str, paddle.nn.Layer, int, LayerInfo | None]] = [
        ...
        if input_data is None or isinstance(module, WRAPPER_MODULES):
            ...
            if module_id in hooks:
                for hook in hooks[module_id]:
                    hook.remove()
            hooks[module_id] = module.register_forward_pre_hook(
                hook=pre_hook
            ), module.register_forward_post_hook(
                hook=construct_hook(global_layer_info, batch_dim)
            )
        stack += [
            (name, mod, curr_depth + 1, global_layer_info[module_id])
            for name, mod in reversed(module._sub_layers.items())
            if mod is not None
        ]
