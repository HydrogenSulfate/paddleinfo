...
class FactorizedReduce(paddle.nn.Layer):
    ...
    def forward(self, x):
        x = self.relu(x)
        out = paddle.cat([self.conv_1(x), self.conv_2(x[:, :, 1:, 1:])], dim=1)
        out = self.bn(out)
        return out
...
class Cell(paddle.nn.Layer):
    ...
    def forward(self, s0, s1, drop_prob):
        ...
        for i in range(self._steps):
            ...
            s = h1 + h2
            states += [s]
        return paddle.cat([states[i] for i in self._concat], dim=1)


class GenotypeNetwork(paddle.nn.Layer):
    def __init__(self, C=16, num_classes=10, layers=1, auxiliary=False):
        ...
        self.cells = paddle.nn.LayerList()
